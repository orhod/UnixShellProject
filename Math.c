...
int main(int argc, char * argv[]){
    ...
    char buff[BUFF], directory[BUFF] = "./Commands/Math";
    struct stat st = {0};
    
    // Create a 'Math' folder if it doesn't exist
    if (stat(directory, &st) == -1) {
        mkdir(directory, 0700);
    }
    strcat(directory, "/Math_Commands.txt");
	...
	if(pid == 0){
        execlp("./Math_shell", "Math_shell", NULL);
        printf("Error in Math_shell\n");
        exit(-1);
    }
