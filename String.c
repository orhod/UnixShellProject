...
int main(int argc, char * argv[]){
    ...
    char buff[BUFF], directory[BUFF] = "./Commands/String";
    struct stat st = {0};
    
    // Create a 'String' folder if it doesn't exist
    if (stat(directory, &st) == -1) {
        mkdir(directory, 0700);
    }
    strcat(directory, "/String_Commands.txt");
	...
	if(pid == 0){
        execlp("./String_shell", "String_shell", NULL);
        printf("Error in String_shell\n");
        exit(-1);
    }
